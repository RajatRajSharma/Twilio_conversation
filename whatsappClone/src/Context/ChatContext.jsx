  ...
  useEffect(() => {
    ...
    async function fetchData() {
      await axios
        ...
        .then((response) => { setListofusers(response.data); socketRef.current.emit('NewUsers', response.data) })
    ...
    socket.on("unreadMessages", ({ newMessage, unreadmsg , ListofUsers }) => {
      ...
      if (activeService === 'mail') {
        ...
        let data = ListofUsers.find((user) => user.Email === newMessage.sender_id);
        console.log("DATA : ",data);
        phone = data.phoneNumber;
      }
      setUnreadcount((prevUnreadCount) => {
        const updatedUnreadCount = prevUnreadCount.map(item => {
          if (item.phone === ((activeService === 'mail') ? phone : newMessage.sender_id)) {
            return { ...item, unreadCount: unreadmsg };
          }
          return item;
        });
        if (!updatedUnreadCount.some(item => item.phone === ((activeService === 'mail') ? phone : newMessage.sender_id))) {
          updatedUnreadCount.push({ phone: ((activeService === 'mail') ? phone : newMessage.sender_id), unreadCount: unreadmsg });
        }
